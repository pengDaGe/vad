...
class OrtIsolateSession {
  ...
  final _outputController = StreamController<List<Map<String, dynamic>>>.broadcast();

  IsolateSessionState get state => _state;
  var _state = IsolateSessionState.idle;
  var _initialized = false;
  final _completer = Completer();
  Completer<void> _runLock = Completer<void>()..complete(); // Start completed
  ...
  static Future<void> createNewIsolateContext(
    ...
    await for (final _IsolateSessionData data in newIsolateReceivePort) {
      try {
        ...
        // Recreate input tensors from values in the isolate
        ...
        for (final entry in data.inputValues.entries) {
          ...
          try {
            if (value is int) {
              // Integer scalar (like sample rate)
              ...
              throw Exception('Unexpected input type for $key: ${value.runtimeType} = $value');
            }
          } catch (e) {
        ...
        // Release input tensors created in isolate
        for (final input in inputs.values) {
          input.release();
        }
        
        // Extract values in the isolate before sending
        ...
        for (int i = 0; i < outputs.length; i++) {
          ...
          try {
            // Extract value before releasing
            final value = output.value;
            outputData.add({
              'address': output.address,
              'value': value,
            });
            // Release tensor after extracting value
            output.release();
          } catch (e) {
  ...
  Future<List<OrtValue?>> run(
    ...
    // Wait for any ongoing run to complete
    if (!_runLock.isCompleted) {
      await _runLock.future;
    }
    
    // Create a new lock for this run
    ...
    try {
      ...
      final inputValues = inputs.map((key, value) => MapEntry(key, value.value));
      ...
      await for (final result in _outputController.stream) {
        // Values are already extracted in the isolate, just return them
        final outputs = result.map((map) => _OrtValueProxy(map['value'])).toList();
...
class _IsolateSessionData {
  ...
  final Map<String, dynamic> inputValues; // Add actual values
